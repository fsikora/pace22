...
//TODO ne pas relabel mais donner une valeur entre ? quitte à redecal si trop serré ?
...
bool canBeAddInTopol(const PGraph& G, std::vector<float> S, TNodeI v) {
  for(int e = 0 ; e<v.GetOutDeg() ; e++) {
...
void initialTopSort(const PGraph& G, std::vector<float>& S) {
...
long int sizeDFVS(const PGraph& G, const std::vector<float> S) {
  long int size = 0;
  for (auto v = G->BegNI() ; v<G->EndNI() ; v++) {
    if(S[v.GetId()] == -1) size++;
  }
...
int iplus(const PGraph& G, std::vector<float> S, int v) {
  auto node = G->GetNI(v);
//  if(node.GetOutDeg() > 0) {
...
int iminus(const PGraph& G, std::vector<float> S, int v) {
  int max = 0;
  auto node = G->GetNI(v);
...
std::vector<int> CVminus(const PGraph& G, std::vector<float> S, int v, int i) {
  std::vector<int> sol;
  auto node = G->GetNI(v);
  for(int e=0 ; e<node.GetInDeg() ; e++) {
...
std::vector<int> CVplus(const PGraph& G, std::vector<float> S, int v, int i) {
  std::vector<int> sol;
  auto node = G->GetNI(v);
  for(int e=0 ; e<node.GetOutDeg() ; e++) {
...
int delta(const PGraph& G, std::vector<float> S, int v, int i) {
  return -1 + CVplus(G, S, v, i).size() + CVminus(G, S, v, i).size();
}

void updateVertex(const PGraph& G, std::vector<float> S, int v, int* deltasplus, int* deltasminus, std::vector<bool>& ok) {
...
void applymove(const PGraph& G, std::vector<float>& S, int v, int b,  std::map<float, int>& selectedS, std::vector<int>& candidates, std::vector<bool>& ok) {
...
std::vector<float> recuit(const PGraph& G, std::vector<float>& S, int* deltasplus, int* deltasminus, std::vector<bool>& ok) {
  double T = T0;
  int nbFails = 0;
  //TODO voir comment eviter les float !
  std::vector<float> Sstar(S);
...
int main(int argc, char* argv[]) {
  ...
  std::vector<float> S(G->GetNodes()+1, -1);
